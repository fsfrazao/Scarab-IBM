...
globals[ patch-data trap-data which-transect which-trap]

breed [traps trap]
traps-own [transect-number trap-number]
...
if mouse-down?
[ 
  ask patch mouse-xcor mouse-ycor [if not any? traps-here 
  [sprout-traps 1
    [set shape "circle"
      set color blue
      set size 1]
...
[ 
  ...
  ask patch mouse-xcor mouse-ycor [if not any? traps in-radius min-distance-between-traps
  [sprout-traps 1
    [set shape "circle"
      set color blue
      set size 1
      set transect-number which-transect
      set trap-number which-trap
      set which-trap which-trap + 1]
     ]]]
...
if mouse-down?
[ 
  ask patch mouse-xcor mouse-ycor [if any? traps-here 
  [ask traps-here [die]
    set which-trap which-trap - 1]]]
...
to write-traps
  ...
  foreach sort traps [
    ask ? [
      file-print (word " "  pxcor " " pycor " " transect-number " " trap-number )
    ]
  ]
...
to load-traps
  ...
  [
    ...
    while [ not file-at-end? ]
      [ set trap-data sentence trap-data (list (list file-read file-read file-read file-read)) ]
  ...
  ifelse ( is-list? trap-data )
    [ foreach trap-data [
         create-traps 1 [
             ...
             set size 1
             setxy first ? item 1 ?
             set transect-number item 2 ?
             set trap-number last ?
             set which-trap which-trap + 1]
    ]]  
    [ user-message "You need to load in patch data first!" ]
end
...
292
Mouse
word \"x: \" round(mouse-xcor) word \" y: \" round(mouse-ycor)
0
1
11
...
485
+
set which-transect which-transect + 1\nset which-trap 0
NIL
1
T
OBSERVER
